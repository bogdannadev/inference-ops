...
| `attachment.image` | 1280×1280, `MAX_BODY_BYTES / 8` of base64 | Caddy caps a request body at 4 MB on the paid hostname, and OpenCode resends every image in the history on each turn, so one request must hold several (the image-batches skill uses 6 plus up to 3 crops). OpenCode's default image budget is 5 MB per image — refused every time. It resizes and re-encodes until the base64 fits (`image/image.ts`), so a large screenshot still gets through. The vision tower is patch 16 with `spatial_merge 2`, one token per 32×32 block, so 1280×1280 is ~1,600 charged input tokens. |
