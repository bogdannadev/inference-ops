#!/usr/bin/env bash
# =============================================================================
# Create (or update) the engine.requests usage table (clickhouse/engine-requests.sql).
#
...
docker exec -i qwen36-27b-langfuse-clickhouse \
  ...
  < clickhouse/engine-requests.sql

docker exec qwen36-27b-langfuse-clickhouse \
  clickhouse-client --password "$LANGFUSE_CLICKHOUSE_PASSWORD" \
  -q "SELECT database, name, engine, total_rows FROM system.tables WHERE database='engine'"
